...
from modelos.objetos import Mensaje, Filtro
...
def gestionar_filtros(usuario: Usuario):
    while True:
        print("\n--- Gestión de Filtros ---")
        print("1. Crear nuevo filtro")
        print("2. Listar filtros existentes")
        ...
        if op == "1":
            ...
            while True:
                campo = input(
                    "Campo de filtro ('remitente' o 'asunto'): ").lower()
                if campo in ['remitente', 'asunto']:
                    break
                print("Campo inválido. Debe ser 'remitente' o 'asunto'.")

            valor = input(f"Valor a buscar en '{campo}': ").strip()
            destino = input(
                "Nombre de la carpeta destino (debe existir): ").strip()

            if not usuario.buscar_carpeta(destino):
                print(
                    f"Error: La carpeta '{destino}' no existe. Debe crearla primero.")
                continue

            nuevo_filtro = Filtro(campo, valor, destino)
            usuario.agregar_filtro(nuevo_filtro)
            print(f"Filtro ID {nuevo_filtro.id} creado con éxito.")

        elif op == "2":
            filtros = usuario.obtener_filtros()
            if not filtros:
                print("No hay filtros configurados.")
                continue
            print("\n--- Filtros Existentes ---")
            for f_id, filtro in filtros.items():
                print(f"- {filtro}")
            ...
            try:
                f_id = int(input("Ingrese el ID del filtro a eliminar: "))
                if usuario.eliminar_filtro(f_id):
                    print(f"Filtro ID {f_id} eliminado.")
                else:
                    print(f"No se encontró el filtro con ID {f_id}.")
            except ValueError:
                print("Entrada inválida. Debe ingresar un número.")
        ...
        while intentos < 3:
            ...
            if usuario_encontrado.iniciar_sesion(contraseña):
                ...
                while True:  # menu principal de usuario
                    ...
                    print("7. Gestionar Filtros")  # << NUEVA OPCIÓN
                    print("8. Cerrar sesión")  # << Opción 7 pasa a ser 8
                    op = input("Opción: ")
                    ...
                    if op in ["1", "2"]:
                        ...
                        if servidor_destino is None:
                            print(
                                "El destinatario no existe en ningún servidor. Mensaje NO enviado.")
                            continue

                        if servidor_origen is None:
                            print(
                                "Error interno: no se encontró servidor del remitente.")
                            continue
                    ...
                    # CREAR CARPETA
                        ...
                        if opcion_padre == "1":
                            carpeta_padre = input(
                                "Nombre de la carpeta padre: ").strip()

                        usuario_encontrado.crear_carpeta(nombre, carpeta_padre)
                        print(f"Carpeta '{nombre}' creada exitosamente.")

                    # GESTIONAR FILTROS (NUEVA LÓGICA)
                    elif op == "7":
                        gestionar_filtros(usuario_encontrado)

                    # CERRAR SESIÓN (Opción reenumerada)
                    elif op == "8":
