...
class Mensaje:
    ...
    def __lt__(self, other):
        return int(not self._es_urgente) < int(not other._es_urgente)
...
class Filtro:
    ...
    def __init__(self, campo: str, valor: str, carpeta_destino: str):
        self._id = Filtro._next_id
        Filtro._next_id += 1
        self._campo = campo.lower()  # 'remitente', 'asunto'
        self._valor = valor.lower()
        self._carpeta_destino = carpeta_destino  # Nombre de la carpeta destino
    ...
    @property
    def carpeta_destino(self) -> str:
        return self._carpeta_destino

    def __repr__(self):
        return f"Filtro ID {self.id}: Mover a '{self.carpeta_destino}' si {self.campo} contiene '{self.valor}'"
...
class Carpeta:
    ...
    def buscar_mensaje(self, criterio: str) -> List[Mensaje]:
        ...
        for mensaje in self._mensajes:
            if criterio_lower in mensaje.asunto.lower() or criterio_lower in mensaje.remitente.lower():
                resultados.append(mensaje)
        for subcarpeta in self._subcarpetas.values():
            resultados.extend(subcarpeta.buscar_mensaje(criterio))
        return resultados
